...
for(j in 1:length(str2)){
  for(i in 1:length(timeframe)){
    ...
    data = possibly_riingo_crypto_prices(ticker = symbol, resample_frequency = timeframe[i], start_date = Sys.Date() - (days.back * 1), end_date = Sys.Date() - (days.back*0))
    ...
    }else if(timeframe[i] == "30min"){
      data.add = data.frame(time = seq(ymd_hms(floor_date(Sys.time(), unit = "30 mins")), ymd_hms(Sys.time()) + lubridate::hours(10), by = '30 mins')[2:8],
                            open = NA,
                            high = NA,
                            low = NA,
                            close = NA)
      
    }else if(timeframe[i] == "1hour"){
      data.add = data.frame(time = seq(ymd_hms(floor_date(Sys.time(), unit = "1 hour")), ymd_hms(Sys.time()) + lubridate::hours(24), by = '1 hour')[2:8],
                            open = NA,
                            high = NA,
                            low = NA,
                            close = NA)
      
    }else if(timeframe[i] == "4hour"){
      
      data.add = data.frame(time = seq(ymd_hms(floor_date(Sys.time(), unit = "4 hours")), ymd_hms(Sys.time()) + lubridate::days(10), by = '4 hours')[2:8],
                            open = NA,
                            high = NA,
                            low = NA,
                            close = NA)
      
    }else if(timeframe[i] == 'daily'){
